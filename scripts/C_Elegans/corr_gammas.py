...
for (worm_idx, worm_dataset_path) in enumerate(wt_unc31_worms_datasets_paths_list):
    ...
    for (stim_idx, stim_neu_path) in enumerate(stimulated_neuron_paths_list):
        ...
        try:
            ...
            signal_correlation = np.zeros_like(n_stimuli, n_responding_neurons)
...
signal_correlation_wt_mean    = np.mean(signal_correlation_wt)
signal_correlation_unc31_mean = np.mean(signal_correlation_unc31)

labels = ["Gap junction (WT)", "Gap junction (UNC31)", "Chem. Syn. (WT)", "Chem. Syn. (UNC31)"]
...
ax.set_title("Correlation: Fitted Connections vs Connectome")
ax.grid(axis="y")

plt.savefig(os.path.join(figures_path, "gammas_connectome_vs_fitted_correlation_bar_plot.png"), bbox_inches="tight")
plt.close(fig)
