#
# code for fit of convolution kernels, combination of linear (exponentials) or non-linear (NEGF) kernels considering neural mass model
#
# inspired by 
...
worm_type_path = os.path.join(data_path, "worm_type_"+worm_type)

print("Analysing datasets in ", worm_type_path)
...
for (worm_idx, worm_dataset_path) in enumerate(worms_datasets_paths_list):
    ...
    for (stim_idx, stim_neu_path) in enumerate(stimulated_neuron_paths_list):
        ...
        try:
            ...
            w_total.extend(w_values[valid_indices].flatten())
...
ax[0].scatter(distances, w_values, alpha=0.8, label="gamma_s", color="orange")
ax[0].set_xlabel("r")
ax[0].set_ylabel(r"$w_{ij}$")
ax[0].legend()
ax[0].set_title("coupling vs Distance")
...
# Plot gamma_g vs distance
ax[1].scatter(distances_exp_squared, w_values, alpha=0.8, label="gamma_s", color="orange")
