...
function main()
    ...
    figures_path = "/home/gabriel/figuras/qualification/NEGF_HH/$(C_m)_$(g_Na_bar)_$(g_K_bar)_$(g_L_bar)_$(E_Na)_$(E_K)_$(E_L)_$(I_ext)_$(stim)/"
    mkpath(figures_path)
    ...
    png(plot(tₛ, [V (g_Na_bar*m.^(3).*h) (g_K_bar*n.^4)], layout=(3,1), 
...
#   NONequilibrium direct green functions#
    ...
    for itr=ProgressBar(1:60)  #### iterative method to approximate σ
        ...
        for t=1:resolution
            for t′=1:(t-1) 
                σ_n_V[t, t′]  = (n_inf[t′] - (n0+conv_sigma_n_V[t′])) / (V[t′]*tau_n[t′])
                σ_n_V_const_tau[t, t′]  = (exp(-(t-t′)/tau_n_const)*n_inf[t′]) / (V[t′]*tau_n_const)
            end
        end
                  
        for t=1:resolution
            conv_sigma_n_V[t] = conv(σ_n_V[t, 1:t], V[1:t], tₛ[1:t])# ones(length(1:t)), tₛ[1:t])
            conv_sigma_n_V_const_tau[t] = conv(σ_n_V_const_tau[t, 1:t], V[1:t], tₛ[1:t])# ones(length(1:t)), tₛ[1:t])
        end
