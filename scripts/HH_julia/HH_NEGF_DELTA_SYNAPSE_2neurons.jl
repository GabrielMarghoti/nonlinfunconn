...
function main()
    ...
    figures_path = "/home/gabriel/figuras/qualification/NEGF_HH_DELTA_SYNAPSE_$(N)neurons_excitatory/$(g_Na_bar)_$(g_K_bar)_$(g_L_bar)_$(E_Na)_$(E_K)_$(E_L)_syn_$(minimum(Es))_stim_$(I_ext)_$(stim)/"
    ...
    for j=1:N
        for i=j:N
            for t′=1:resolution
                sigma₀[:, t′, i, j]  = theta.(ts.-ts[t′]) .*a_r[i,j]*(1-gS0[i, j]).*d_alpha_S(V0[j], beta[i,j], V_th[i,j]).*exp.(-(ts.-ts[t′]).*(a_d[i,j]-a_r[i,j]/(1 + exp(-beta[i,j]*(V0[j]-V_th[i,j])))))
                gg₀[:, t′, i, j] = theta.(ts.-ts[t′]) .*A[i, j].*exp.(-(ts.-ts[t′])/(tau_i[i]))
                gs₀[:, t′, i, j] = theta.(ts.-ts[t′]) .*B[i, j].*(Es[i,j]-V0[i]).*exp.(-(ts.-ts[t′])/(tau_i[i]))
            end
            png(
                heatmap(ts, ts, sigma₀[:, :, i, j],
                    xflip=false,
    ...
    for t=1:resolution
        for t′=1:(t-1)
            for i=1:N
                for j=1:N
                    g₀[t, t′, i, j] = gg₀[t, t′, i, j] + conv(gs₀[t, t′:t, i, j], sigma₀[t′:t, t′, i, j], ts[t′:t])
                end
            end
        end
    end
...
#   NONequilibrium green functions
    ...
    for itr=1:10  #### iterative method to approximate sigma
        for j=1:N
            ...
            for t=1:resolution
                for t′=1:(t-1) 
                    𝜋[t, t′, i, j] = conv(gs₀[t, t′:t, i, j], (1 .-(DeltaV[t′:t, i]/(Es[i, j]-V0[i]))).*Chi_s[t′:t, t′, i, j], ts[t′:t])
    ...
    for j=1:N-1
        for i=j:N
            ...
            plote_level_curves_Gamma = plot(ts[end].-ts[2:end], Gamma₀[end, 2:end, i, j],
                            xlabel="t-t′",
                            ylabel = "γ",
                            label = "γ₀",
                            lc=:black,
            ...
            for t_idx in 1:length(ts_plot)
                plot!(ts[ts_plot[t_idx]].-ts[ts_plot[1]:(ts_plot[t_idx])], Gamma[ts_plot[t_idx], ts_plot[1]:(ts_plot[t_idx]), i, j],
                            ...
                            label= "t = "*string(round(ts[ts_plot[t_idx]], digits=2)),
                            size=(600,400),
                            dpi=200, 
                            frame_style=:box,
                            grid=false)
            ...
            plote_level_curves_Gamma = plot(ts[1:end], Gamma₀[end, 1:end, i, j],
                            xlabel="t′",
                            ylabel = "γ",
                            label = "γ₀",
                            lc=:black,
            ...
            colors_codes = create_rgba_code_line(length(ts_plot))
            for t_idx in 1:length(ts_plot)
                plot!(ts[1:(ts_plot[t_idx])], Gamma[ts_plot[t_idx], 1:(ts_plot[t_idx]), i, j].-Gamma₀[ts_plot[t_idx], 1:(ts_plot[t_idx]), i, j],
                            xlabel="t′ (ms)",
