...
function main()
    ...
    ts_plot = [40; 80; 100; 200; 300; 500; 1000]

    figures_path = "/home/gabriel/figuras/qualification/NEGF_HH_DELTA_SYNAPSE_$(N)neurons_excitatory_SINE/$(g_Na_bar)_$(g_K_bar)_$(g_L_bar)_$(E_Na)_$(E_K)_$(E_L)_syn_$(minimum(Es))_stim_$(I_ext)_$(stim)/"
    ...
    frame_style=:box, size=(200*N,500), dpi=200, grid=false)
...
#   NONequilibrium green functions
    ...
    for itr=ProgressBar(1:10)  #### iterative method to approximate σ
        for i=1:N
            for t=1:resolution
                ...
                for t′=1:t
                    Chi_i[t, t′, i] = conv(Χ_K[t, t′:t, i], κ_gK_V[t′:t, t′, i], tₛ[t′:t]) + conv(Χ_Na[t, t′:t, i], κ_gNa_V[t′:t, t′, i], tₛ[t′:t])
                end
            end
            ...
            if itr%10==0 || itr==1
                png(
                    ...
                    lc=[:gray],
                    ls=[:solid],
                    annotation=(2.5, 0.8, "Stimulus")
                    )
                    for t_plot in ts_plot

                        r_color = -4t_plot*(t_plot-resolution)/(resolution*resolution)
                        b_color = -4t_plot*(t_plot-200)/(200*resolution)
                        if b_color<0
                            b_color = 0
                        end

                        plot!(tₛ[1:(t_plot)] , Chi_i[t_plot, 1:(t_plot), i],
                        ylabel= "χᵢ(t,t′)",
                        label="t=$(round(tₛ[t_plot],digits=1))",
                        xlabel = "t′ (ms)",
                        size=(600,500),
                        lc=RGBA(r_color, 0, b_color, 1),
                        ls=[:solid],
                        lw=2.5,
                        dpi=200, 
                        frame_style=:box,
                    ...
                    plot!(tₛ[1:(end)] , Chi_i[end, 1:(end), i],
                    ylabel= "χᵢ(t,t′)",
    ...
    for itr=1:10  #### iterative method to approximate σ
        ...
        plot_Delta_S_sigma_V = plot(tₛ, [ΔgS[:, 2, 1]],     # plot only the synapses that matter for the networks (4←3←2←1)
        ylabel=["Δgˢ₂₁"],
        label="Simulated",
        size=(500,400),
        lc = :black,
        ...
        frame_style=:box,
        grid=false)
        plot!(tₛ, [conv_sigma_V[:, 2, 1]],     # plot only the synapses that matter for the networks (4←3←2←1)
        label="ηˢ₂₁ ∗ ΔV₁",
        lc = :red,
        ls=:dot,
        lw=2,
    ...
    for i=1:N
        for t=1:resolution
            for j=1:N
                conv_Gamma_V[t, i] += conv(g[t, 1:t, i, j], ΔV[1:t, j], tₛ[1:t])
            end
        end
    end
        ...
        for i=j:N
            png(
                heatmap(tₛ, tₛ, Gamma₀[:, :, i, j],
                    ...
                    frame_style=:box,
                    grid=false),
                    figures_path*"neuron$(i)/Gamma0_ij$(i)_$(j)"
                )
            png(
                heatmap(tₛ, tₛ, Gamma[:, :, i, j],
                    xflip=false,
                    ...
                    frame_style=:box,
                    grid=false),
                    figures_path*"neuron$(i)/Gamma_ij$(i)_$(j)"
                )
            ...
            for t_idx in ts_plot
                plot!(tₛ[t_idx].-tₛ[ts_plot[1]:(t_idx)], Gamma[t_idx, ts_plot[1]:(t_idx), i, j],
                            xlabel="t-t′",
                            ylabel = "γᵢⱼ",
                            ...
                            dpi=200, 
                            frame_style=:box,
                            xlims = (0,10),ylims=(-0.5, 1.0),
                            grid=false)
                    png(plote_level_curves_Gamma,
                            figures_path*"neuron$(i)/Gamma_level_curves_ij$(i)_$(j)"
                        )
            end
             plote_level_curves_Gamma_over_chi = plot(tₛ[end].-tₛ[2:end], Gamma₀[end, 2:end, i, j]./Chi_i[end, 2:end, i],
                            xlabel="t-t′",
                            ylabel = "γ",
                            label = "",
                            lc=:black,
                            ...
                            grid=false)
            for t_idx in ts_plot
                plot!(tₛ[t_idx].-tₛ[ts_plot[1]:(t_idx)], Gamma[t_idx, ts_plot[1]:(t_idx), i, j]./Chi_i[t_idx, ts_plot[1]:(t_idx), i],
                            xlabel="t-t′",
                            ylabel = "γᵢⱼ",
                            lc = RGBA(t_idx/ts_plot[end], 0, 1 - (2*t_idx/ts_plot[end] -1)^2, 1),
                            la=0.9,
                            ...
                            frame_style=:box,
                            grid=false,
                            xlims = (0,10),ylims=(-0.2, 0.2))
                    png(plote_level_curves_Gamma_over_chi,
                            figures_path*"neuron$(i)/Gamma_over_CHI_level_curves_ij$(i)_$(j)"
                        )
            end
        end
    end
