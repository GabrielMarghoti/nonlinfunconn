...
function hh_model!(du, u, p, t)
    ...
    for i=1:N # Iteration over neurons variables
        ...
        for k =1:length(stim_nodes)
            if (t>=5.0 && t<=11.0) && i == stim_nodes[k] 
                I_ext += stimuli[k]
            end
        end
...
function main()
    ...
    g_Na_bar =  0#120.0         # maximum conductances, in mS/cm^2
    g_K_bar  =  0#36.0
    g_L_bar  =   0.5
    ...
    figures_path = "/home/gabriel/figuras/qualification/NEGF_LIF_DELTA_SYNAPSE_$(N)neurons_LINEAR_2PATHS_tf$(tf)/syn_$(minimum(Es))_stim_$(I_ext)/"
    
    for i=1:N
        mkpath(figures_path*"neuron$(i)/")
    end

    data_path = "/home/gabriel/dados/qualification/NEGF_LIF_DELTA_SYNAPSE_$(N)neurons_LINEAR_2PATHS_tf$(tf)/syn_$(minimum(Es))_stim_$(I_ext)/"
