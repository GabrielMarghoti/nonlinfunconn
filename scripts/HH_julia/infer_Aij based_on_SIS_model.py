...
if __name__ == "__main__":
    ...
    def nonlinear_equation_stimuli(t, X, N, A):
        ...
        for i in range(N):
            ...
            for j in range(N):
                sum_term += A[i, j] * (1 - X[i]) * X[j]
            stim = 1 if i == 3 and 2 < t < 8 else 0  # Stimulus condition
            dX[i] = sum_term - X[i] + stim
        return dX

    sol = solve_ivp(lambda t, X: nonlinear_equation_stimuli(t, X, N, A), t_span, Xeq, t_eval=t_eval)

    DeltaX = sol.y +  np.random.normal(0, 0.05, (N, resolution)) - np.tile(Xeq.reshape(-1, 1), (1, len(sol.t)))  # Add random fluctuation
