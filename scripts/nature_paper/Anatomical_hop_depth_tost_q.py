...
_, qvalues = funa.get_kolmogorov_smirnov_q(inclall_occ2,return_tost=True,tost_th=1.2)
...
labels = ['1','2','3','4']
ax.bar(labels, probs, color="C1")
plt.xticks(np.arange(0, 4, step=1), fontsize= 25)
ax.set_xlabel('Min Anatomical Path Length',fontsize= 25)
ax.set_ylabel('Prob of q<0.05\ngiven $l$',fontsize= 25)
ax.set_yticks([0.0,0.2,0.4,0.6])
ax.set_yticklabels(["0.0","0.2","0.4","0.6"],fontsize=25)
#ax.set_ylim(0,0.18)
ax.spines['right'].set_visible(False)
ax.spines['top'].set_visible(False)
np.savetxt("plots/AconnHopDepth2_tost.txt", np.array([np.array(labels).astype(float), probs]))
plt.savefig("plots/AconnHopDepth2_tost.pdf", bbox_inches='tight')
...
np.savetxt("plots/AconnHopDepth_tost.txt", np.array([min_anatomical_hops_flat,min_anatomical_hops_all_flat]))
plt.savefig("plots/AconnHopDepth_tost.pdf", bbox_inches='tight')
