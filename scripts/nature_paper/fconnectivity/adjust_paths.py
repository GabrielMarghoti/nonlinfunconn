...
your_folder = os.getcwd()
if your_folder[-1] != "/": your_folder+="/"

ds_list = your_folder+"ds_list_full_orig.txt"
...
f = open(your_folder+"ds_list_full.txt","w")
...
for ds in np.arange(110, len(folders)):
    # Get reference volume index
    match_info = wormb.match.load_match_parameters(folders[ds])
    ...
    print(mc_cerv_folder)

    if mc_cerv_folder is not None:
        ...
        if 'multicolorworm' not in mc_cerv_folder_partial : 
            mc_cerv_folder_partial = mc_cerv_folder.split("/")[-1]

        print(mc_cerv_folder.split("/"))
        mc_cerv_folder = your_folder+'exported_data_full/'+mc_cerv_folder_partial+"/"
