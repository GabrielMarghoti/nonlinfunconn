...
if export_to_txt:
    if ds_tags=="unc31":
        funa.export_to_txt("/exported_data_unc31/")
    elif ds_tags is None and ds_exclude_tags=="mutant":
        funa.export_to_txt("/exported_data/")
    quit()
...
if plot:    
 ...
 #   if stamp: pp.provstamp(ax,-.1,-.1," ".join(sys.argv))            # commented
...
if save:
    ...
    if to_paper and plot:
        plt.savefig("/home/gabrielm/paper_reproduction/plots/funatlas_intensity_map.pdf", dpi=600, bbox_inches="tight")
if dst is not None:
    print(dst[-4:])
