...
for i_excl_ds in np.arange(n_excl_ds):
    if excl_ds[0] is not None:
        txt_fname = "plots/funatlas_intensity_map_excl_"+str(i_excl_ds)+add_s+".txt"
    else:
        txt_fname = "plots/funatlas_intensity_map_cache"+add_s+".txt"
        txt_fname_q = "plots/funatlas_intensity_map_cache_q"+add_s+".txt"
    F = np.loadtxt(""+txt_fname)
    F[np.isnan(F)] = 0.0
    q = np.loadtxt(""+txt_fname_q)
    F[q>0.05] = 0.0
    ...
    if excl_ds[0] is None:
        np.savetxt("plots/funatlas_intensity_map_inverted"+add_s+".txt",f[0])
...
if excl_ds[0] is None:
    ...
    np.savetxt("plots/funatlas_intensity_map_inverted"+add_s+".txt",f_body)
    plt.figure(6)
    plt.imshow(f_body)
    plt.show()
...
if excl_ds[0] is not None:
    ...
    fig3.savefig("plots/F_to_f_inversion_steady_state"+add_s+".png",bbox_inches="tight")
    plt.show()
