...
try:
    fig.tight_layout()
    fig.savefig("plots/"+folder.split("/")[-2]+"_waterfall.png", dpi=300, bbox_inches="tight",metadata={"Comment":" ".join(sys.argv)})
    fig.savefig("plots/"+folder.split("/")[-2]+"_waterfall.pdf", dpi=300, bbox_inches="tight")
    if to_paper:
        fig.savefig("plots/"+folder.split("/")[-2]+"_waterfall.pdf", dpi=300, bbox_inches="tight")
except:
