...
n_regions = np.array([28, 34, 35, 36, 44, 45]).shape[0] # data.shape[1]
...
lif_gf = nlfc.GreenFunctions(
    ...
    x = data[1:3, np.array([28, 34, 35, 36, 44, 45]), 0:fitting_window:lowering_fit_resolution_step],
    dt = dt,
)
...
spondata = spondata[np.array([28, 34, 35, 36, 44, 45]), :]
...
for i in np.arange(0, n_regions):
    ...
    for j in range(n_regions):
        ...
        predspondata[i] += nlfc.utils.nontt_conv(lif_gf.g[0][i, j], delta_j, dt=dt)
        
        if (np.all(abs(lif_gf.g[0][i, j]) < 1e-02)): 
            continue
        #nlfc.utils.plots.t_t_heatmap(x, g[i, j, :, :], os.path.join(ie_dir, f'negf_g_heatmap_neuron_pair_{i}_{j}_stimulation_{str(ie)}.png'))
        
        nlfc.utils.plots.time_level_curves( dt*np.arange(time_len), lif_gf.g[0][i, j], lif_gf.g[0][i, j,  -1], xlabel=None, ylabel="g(t,t')", title=f"Neurons : {i}<-{j}", save_path= os.path.join(output_fig_dir,f'fitted_negf_direct_g_neurons_neuron_pair_{i}<-{j}.png'))
